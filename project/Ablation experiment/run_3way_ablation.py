...
# 3. 核心评估流程
# =========================
def run_3way_eval(seed, device):
    print(f"\n[INFO] 正在初始化 Seed {seed} 的终极三维消融评估 (CoT vs No-CoT vs Empty)...")
    ...
    if "sequence_id" in val_cot.columns:
        ...
        print("[INFO] Sequence ID 严格对齐检查通过！")
    ...
    # --- C. 提取 BGE 特征 ---
    print(f"[INFO] 正在并行提取三组 BGE 特征 (此过程约需1分钟)...")
    ...
    H_nocot_val = extract_bge(val_nocot)
    H_nocot_te = extract_bge(test_nocot)
    ...
    def optimize_and_fuse(Z_llm_val, Z_llm_te):
        ...
        return best_alpha, best_mrr, Z_fused_te

    alpha_cot, val_mrr_cot, Z_fused_cot = optimize_and_fuse(Z_cot_val, Z_cot_te)
    alpha_nocot, val_mrr_nocot, Z_fused_nocot = optimize_and_fuse(Z_nocot_val, Z_nocot_te)
    ...
    metrics_llm_nocot = calc_metrics(Z_nocot_te, y_te)
    metrics_llm_empty = calc_metrics(Z_empty_te, y_te)

    metrics_fus_cot = calc_metrics(Z_fused_cot, y_te)
    metrics_fus_nocot = calc_metrics(Z_fused_nocot, y_te)
    ...
    # --- G. 打印三维战报大表 ---
    print(f"\n{'=' * 110}")
    print(f"🔥 LLM Reasoning 终极三维消融战报 (Seed {seed}) | 测试样本数: {n}")
    print(
    ...
    print(f"   -> [No-CoT] 最优 Alpha: {alpha_nocot:.2f} (Val MRR: {val_mrr_nocot:.4f})")
    ...
    print_row("No-CoT", metrics_llm_nocot, metrics_fus_nocot)
    print_row("Empty", metrics_llm_empty, metrics_fus_empty)
    print(f"{'=' * 110}")

    # --- H. 打印极度直观的增益对比 ---
    print("\n[关键结论 1: CoT 相对 No-CoT 的增益 (证明显式推理链的价值)]")
    print(f"  -> LLM-only  Top-1 Δ: {metrics_llm_cot[0] - metrics_llm_nocot[0]:+.4f}")
    print(f"  -> LLM-only  Top-5 Δ: {metrics_llm_cot[1] - metrics_llm_nocot[1]:+.4f}")
    print(f"  -> LLM-only  MRR   Δ: {metrics_llm_cot[2] - metrics_llm_nocot[2]:+.4f}")
    print(f"  -> Fusion    Top-1 Δ: {metrics_fus_cot[0] - metrics_fus_nocot[0]:+.4f}")
    print(f"  -> Fusion    Top-5 Δ: {metrics_fus_cot[1] - metrics_fus_nocot[1]:+.4f}")
    print(f"  -> Fusion    MRR   Δ: {metrics_fus_cot[2] - metrics_fus_nocot[2]:+.4f}")

    print("\n[关键结论 2: CoT 相对 Empty 的绝对增益 (证明语义信息的整体价值)]")
    ...
    print("\n[关键结论 3: No-CoT 相对 Empty 的增益 (证明短结论文本也携带语义信息)]")
    print(f"  -> LLM-only  Top-1 Δ: {metrics_llm_nocot[0] - metrics_llm_empty[0]:+.4f}")
    print(f"  -> LLM-only  Top-5 Δ: {metrics_llm_nocot[1] - metrics_llm_empty[1]:+.4f}")
    print(f"  -> LLM-only  MRR   Δ: {metrics_llm_nocot[2] - metrics_llm_empty[2]:+.4f}")
    print(f"  -> Fusion    Top-1 Δ: {metrics_fus_nocot[0] - metrics_fus_empty[0]:+.4f}")
    print(f"  -> Fusion    Top-5 Δ: {metrics_fus_nocot[1] - metrics_fus_empty[1]:+.4f}")
    print(f"  -> Fusion    MRR   Δ: {metrics_fus_nocot[2] - metrics_fus_empty[2]:+.4f}")
    print(f"{'=' * 110}")


if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    run_3way_eval(42, device)
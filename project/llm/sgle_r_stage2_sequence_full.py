...
def main() -> None:
    ...
    parser.add_argument(
        "--boundary_txt",
        type=str,
        default=str(LOGS_DIR / "sgle_r_stage2_full_tokenizer_boundary.txt"),
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="0 means use all rows in stage1_jsonl",
    )
    parser.add_argument(
        "--beta",
        type=float,
    ...
    for row_idx, row in enumerate(stage1_rows):
        ...
        for cand_tid in all_tids:
            cand_name = parent_tid_to_name.get(cand_tid, "")
            label_text = f"{cand_tid} - {cand_name}" if cand_name else cand_tid
            candidate_text = (
                "Answer:\n"
                 f"The most likely immediate next ATT&CK parent technique is {label_text}."
            )
            ...
            boundary_lines.append(
                f"instance_id={instance_id} | sample_id={sample_id} | candidate_tid={cand_tid} | "
        ...
        for rank, (cand_tid, score) in enumerate(sample_scores[:5], start=1):
            cand_name = parent_tid_to_name.get(cand_tid, "")
            cand_text = f"{cand_tid} - {cand_name}" if cand_name else cand_tid
            print(f"rank={rank} candidate={cand_text} score={score:.6f}")

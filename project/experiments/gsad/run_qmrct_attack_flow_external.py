...
def run_attack_flow_external(
    ...
    external, data_audit = load_attack_flow_transitions(
        project_root=root_path, vocab=vocab, exclude_overlapping_ctid=True
    )
    qmrct_config = {
    ...
    predictions = pd.DataFrame(
        {
            ...
            "flow": external["flow"].astype(str).to_numpy(),
    ...
    grouped = predictions.rename(columns={"flow": "root"})
    metrics = {
        "baseline_flow_macro_top1": root_macro_mean(
            predictions, "baseline_correct", "flow"
        ),
        "candidate_flow_macro_top1": root_macro_mean(
            predictions, "candidate_correct", "flow"
        ),
        "top1_gain_pp": 100.0
        * _gain(grouped, "candidate_correct", "baseline_correct"),
        "baseline_flow_macro_mrr": root_macro_mean(
            predictions, "baseline_rr", "flow"
        ),
        "candidate_flow_macro_mrr": root_macro_mean(
            predictions, "candidate_rr", "flow"
        ),
        ...
        "flows": float(predictions["flow"].nunique()),
    }
    intervals = {
    ...
    if output_dir is not None:
        ...
        flow_metrics.to_csv(destination / "flow_metrics.csv", index=False)
        ...
        write_manifest(
            ...
            inputs={
                "repository_commit": "295d20d27cefce0a2d309b6c24781545e45f547d",
                "license_sha256": sha256_file(corpus_root / "LICENSE"),
            ...
            split_audit={
                ...
                "external_flows": external["flow"].nunique(),
                "excluded_overlap_flows": sorted(
                    ["Turla - Carbon Emulation Plan", "Turla - Snake Emulation Plan"]
                ),
            },
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    destination = args.output_dir or (
        ...
        / "results"
        / "external"
        / f"qmrct_attack_flow_seed{args.seed}"
    )
    _, metrics = run_attack_flow_external(

...
1. `LLM_semantic_future3_lodo_validation_v8.md`；
2. SECRYPT 正式论文/代码计数一致性审计；
3. `P-pair/P-campaign/P-source` 三协议冻结说明与重复率报告；
4. SECRYPT clean-room 适配说明；
5. Unit42 原始 playbook 可行性审计报告；
6. future-3 三源/四源冻结数据与 manifest；
7. TIE-local、Kuwano、LSTM、Transformer、Markov beam 和 SECRYPT-adapted HM 基线报告；
8. 新 30 条开发试点报告；
9. 全量语义实验和逐样本结果；
10. campaign 统计、分层和最终成立/判死报告；
11. 论文方法与实验章节。

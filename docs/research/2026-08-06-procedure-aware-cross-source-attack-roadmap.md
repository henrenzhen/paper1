...
> 状态：三源 `P-source` 主实验、30条开发试点、784条正式DeepSeek生成、HM+S/HM+P/HM+T/HM+ST、直接排名融合和候选级RGAF可学习性审计均已完成；预注册融合与事后数值门控均缺乏正向证据。
> 最近修订：DeepSeek直接Top-5 `B0` 的来源等权NDCG@5为0.2028；事后探索`T+B0`为0.2053，相对B0仅+0.0025且95% CI [-0.0007,+0.0078]。进一步纠正了把真值条件`transition_visibility`误当推理特征的问题，并冻结合法候选级RGAF：结果为0.2002，相对B0为-0.0027，95% CI [-0.0044,-0.0012]；置换证据控制精确保持B0的0.2028，统一开启残差仅0.1667。Unit42的153个campaign中0个具有合格技术顺序。当前证据支持“LLM直接排序含跨来源信号”，但不支持摘要probe、固定权重或当前可观测候选支持门控提供稳定互补增益。
...
15. CTID的10-campaign leave-one-campaign-out影响审计显示：B0-K、B0-T和B0-HM的NDCG@5差值在删除任意一个campaign后仍为正；B0-A0的完整差值为+0.0188，但删除`fin6`后为-0.0017。故B0优于复杂序列基线的CTID方向不由单一campaign决定，但“稳定优于频率先验”不能在CTID单源上成立。T+B0-B0在CTID仅+0.0001且会翻转。
16. `transition_visibility`由真实future-3目标与训练转移共同计算，只能用于解盲后的机制分层，不能作为推理时门控输入。正确来源等权复算中，“all_seen用T、其余用B0”为0.2098，“all_seen用T、mixed用K、其余用B0”为0.2221；它们仍读取真值分层，只是诊断上界，不是方法结果。
17. 冻结的合法候选级RGAF只使用训练转移计数、候选支持、分支不确定性和前缀长度，并以campaign-LOO证据训练门控。其来源等权NDCG@5为0.2002，相对B0为-0.0027，95% CI [-0.0044,-0.0012]；CTID和Attack Flow均为负。候选置换控制为0.2028，统一开启转移残差为0.1667。故当前候选支持特征未学会恢复真值条件oracle空间。
18. RGAF的784行、10/35/27个campaign、输入/脚本/协议/输出哈希均通过独立审计；在另一目录完整重跑后，八个托管输出逐字一致。该结果是可复现的事后方法开发证据，但不能升级为前瞻确认。

至此，已经排除了“简单加容量”“原始文本直接编码”“摘要embedding probe”“战术软先验”“Markov beam约束”“LSTM-Markov混合”“固定排名融合”和“浅层候选支持门控”等方案。核心融合方法在当前三源任务设定下缺乏证据；但B0表明问题不等于“LLM完全无信号”。若继续保留方法路线，下一种实质不同的融合必须让LLM直接读取训练来源统计证据并重排受限候选，而不能继续在相同784行上搜索新的数值权重后把结果当确认。
...
**状态：核心判定与候选级门控审计均已完成。** campaign bootstrap、seen/unseen、文本长度和目标基数分层均已落盘；HM+S判死与HM+ST互补性不成立已经封口。T+B0没有检出相对B0的可靠增量；合法候选级RGAF显著低于B0。CTID leave-one-campaign influence已完成，其他新来源上的前瞻确认仍待完成。
...
- 后续合法候选级RGAF也不构成翻案：RGAF-B0=-0.0027，95% CI完全低于0，且RGAF低于候选置换控制。

正式结论为：**已测试的LLM语义融合在当前任务设定下缺乏证据。** 这不否定B0直接LLM排序的独立信号，也不等价于所有可能的LLM--转移交互都不可能；但禁止把B0、真值条件oracle或事后门控替换成预注册主方法来改写主结论。
...
已完成：v8/v8.1、三源future-3数据、SECRYPT划分审计、P-source主要基线、30条试点、784条全量生成、S/P/T/ST实验、直接排名融合、CTID影响分析、候选级RGAF审计、逐样本结果和核心判定。

后续按证据价值排序：

1. 停止在同一784行上追加数值门控并把最好结果当确认；真值条件oracle只保留为上界诊断。
2. 若坚持方法路线，先冻结一个**转移证据增强的直接LLM重排器**：候选仅来自B0与训练来源A/T/K的固定并集，LLM读取训练计数与跨训练源支持后输出Top-5；它与已经失败的摘要probe、固定权重和浅层门控有实质区别。
3. 新重排器先在排除于正式评估的30条开发样本上做机械试点。任何新prompt、发送字段、样本数和费用都需要重新获得API授权；试点不能用于宣称效果。
4. 搜索其他具有显式事件顺序和procedure文本的新来源；Unit42 bundle已判定不合格。若从Unit42对应叙事报告重建，必须先冻结报告选择、事件抽取、分支线性化和独立group规则。
5. 只有在真正未参与本轮方法选择的新来源上，才可前瞻验证B0与新重排器并形成方法主张；同一784行上的结果只能标为探索性开发。
6. 完成TIE-local/KUW原式核验、`P-pair/P-campaign`神经模型和统一technique/tactic coverage矩阵，补齐相关工作公平对比。
